...
class FlashInferCutlassMxfp8LinearKernel(Mxfp8LinearKernel):
    ...
    FlashInfer requires both matrix dimensions to be at least 128. On SM120,
    narrower output projections retain their serialized MXFP8 representation
    and use B12X instead. This permits mixed-shape models to use FlashInfer for
    every supported projection without padding narrow outputs or dequantizing
    their weights.
    """
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if N < self._MIN_GEMM_DIM:
            from .b12x import B12xMxfp8LinearKernel

            supported, reason = B12xMxfp8LinearKernel.is_supported()
            if not supported:
                raise ValueError(
                    "FlashInfer CUTLASS MXFP8 requires output features >= "
                    f"{self._MIN_GEMM_DIM}, got {N}; the B12X narrow-output "
                    f"fallback is unavailable: {reason}"
                )
            fallback = B12xMxfp8LinearKernel(self.config)
            fallback.process_weights_after_loading(layer)
            layer._mxfp8_uses_b12x_narrow_output = True
            return
    ...
    def apply_weights(
        ...
        if getattr(layer, "_mxfp8_uses_b12x_narrow_output", False):
            from .b12x import _apply_b12x_mxfp8_packed_linear
