...
def init_attn_backend(
    ...
    shared_gdn_metadata_builders: dict[
        tuple[int, int], GDNAttentionMetadataBuilder
    ] = {}
    for kv_cache_group_id, groups in enumerate(attn_groups):
        ...
        for group in groups:
            ...
            # Exact GDN builders use the same request-level metadata for every
            # cache group.  Subclasses retain private buffers because they may
            # extend the metadata contract.
            if type(builder) is GDNAttentionMetadataBuilder:
                sharing_key = (
                    builder.num_spec,
                    builder.decode_cudagraph_max_bs,
                )
                if owner := shared_gdn_metadata_builders.get(sharing_key):
                    builder.share_cudagraph_common_buffers(owner)
                else:
                    shared_gdn_metadata_builders[sharing_key] = builder
