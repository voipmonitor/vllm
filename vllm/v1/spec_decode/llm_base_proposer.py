...
class SpecDecodeBaseProposer:
    ...
    def propose(
        ...
        for token_index in range(self.num_speculative_tokens - 1):
            ...
            # Rebuild or update attention metadata. When draft positions are
            # constant (e.g. Gemma4 MTP), common_attn_metadata is invariant
            # across loop iterations so we build once and reuse. Otherwise,
            # metadata builders that support in-place drafting updates avoid
            # rebuilding all per-layer metadata for every MTP step.
            if self.constant_draft_positions and token_index > 0:
                pass
            elif (
                can_reuse_draft_attn_metadata
                and reuse_draft_attn_metadata_ready
            ):
