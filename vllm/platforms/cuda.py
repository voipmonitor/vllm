    ...
    def get_attn_backend_cls(
        ...
        if len(valid_backends_priorities) == 0:
            if attn_selector_config.use_sparse:
                ...
                fallback_config = attn_selector_config._replace(use_sparse=False)
                valid_backends_priorities, fallback_reasons = cls.get_valid_backends(
                    device_capability=device_capability,
                    attn_selector_config=fallback_config,
                    num_heads=num_heads,
                )
                all_invalid_reasons.update(fallback_reasons)
            if len(valid_backends_priorities) == 0:
                raise ValueError(
