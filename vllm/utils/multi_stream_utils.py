...
def _is_tracing() -> bool:
    """Return true during torch.compile/export tracing.

    CUDA Event and Stream operations are not valid FX graph nodes, so callers
    using eager multi-stream overlap must fall back to sequential execution
    while tracing. Runtime execution can still use the multi-stream fast path.
    """
    try:
