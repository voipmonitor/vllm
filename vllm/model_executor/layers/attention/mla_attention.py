...
from vllm.v1.attention.ops.dcp_alltoall import (
    dcp_a2a_lse_reduce,
    dcp_flashinfer_a2a_lse_reduce,
    init_flashinfer_dcp_a2a_workspace,
)
...
_DEBUG_DCP_LSE = os.getenv("VLLM_DEBUG_DCP_LSE", "0") == "1"
_DEBUG_DCP_LSE_LIMIT = int(os.getenv("VLLM_DEBUG_DCP_LSE_LIMIT", "512"))
_DEBUG_DCP_GRAPH_STATS = os.getenv("VLLM_DEBUG_DCP_GRAPH_STATS", "0") == "1"


def _record_debug_stats(buffer: torch.Tensor | None, offset: int, tensor: torch.Tensor | None) -> None:
    if buffer is None or tensor is None or buffer.numel() < offset + 4:
        return
    ...
    buffer[offset : offset + 4].copy_(stats)
_DCP_COMM_SYNC_DEBUG = os.getenv("VLLM_DCP_COMM_SYNC_DEBUG", "0") != "0"
...
def _dcp_comm_sync_debug(stage: str) -> None:
    if _DCP_COMM_SYNC_DEBUG and torch.cuda.is_available():
        torch.cuda.synchronize()
        logger.warning("VLLM_DCP_COMM_SYNC_DEBUG passed %s", stage)


@functools.lru_cache(maxsize=None)
def _is_arch_support_pdl() -> bool:
    """Programmatic Dependent Launch requires SM >= 90 (Hopper / Blackwell).

    Mirrors sglang's `is_arch_support_pdl()` (sgl_kernel/utils.py) so we can
    pass `enable_pdl=` to FlashInfer kernels that accept it. Cached because
    device capability is fixed for the lifetime of the process.
    """
...
class MLAAttention(nn.Module, AttentionLayerBase):
    ...
    def __init__(
        ...
        **extra_impl_args,
        ...
        # FlashMLA/B12X Sparse Attention fp8 backends use DeepSeek's
        # "fp8_ds_mla" packed KV-cache format.
        ...
        self.dcp_flashinfer_a2a = (
            vllm_config is not None
            and vllm_config.parallel_config.decode_context_parallel_size > 1
            and vllm_config.parallel_config.dcp_comm_backend == "flashinfer_a2a"
        )
        if self.dcp_flashinfer_a2a:
            init_flashinfer_dcp_a2a_workspace(get_dcp_group())
        ...
        if _DEBUG_DCP_GRAPH_STATS:
            self.register_buffer(
                "_debug_dcp_graph_stats",
                torch.zeros(20, dtype=torch.float32),
                persistent=False,
            )
        else:
            self._debug_dcp_graph_stats = None
        ...
        if num_mqa_tokens > 0:
            ...
            if self.impl.dcp_world_size > 1:
                if fp8_attention and self.impl.supports_quant_query_input:
                    ...
                    _dcp_comm_sync_debug("mla_decode.before_q_all_gather_fp8")
                    mqa_q = get_dcp_group().all_gather(mqa_q, dim=1)
                    _dcp_comm_sync_debug("mla_decode.after_q_all_gather_fp8")
                else:
                    mqa_q = torch.cat((mqa_ql_nope, mqa_q_pe), dim=-1)
                    _dcp_comm_sync_debug("mla_decode.before_q_all_gather")
                    mqa_q = get_dcp_group().all_gather(mqa_q, dim=1)
                    _dcp_comm_sync_debug("mla_decode.after_q_all_gather")
                if _DEBUG_DCP_GRAPH_STATS:
                    _record_debug_stats(self._debug_dcp_graph_stats, 0, mqa_q)
            ...
            if _DEBUG_DCP_GRAPH_STATS and self.impl.dcp_world_size > 1:
                _record_debug_stats(self._debug_dcp_graph_stats, 4, attn_out)
                _record_debug_stats(self._debug_dcp_graph_stats, 8, lse)
            ...
            if (
                ...
                debug_count = getattr(self, "_debug_dcp_lse_count", 0)
                if debug_count < _DEBUG_DCP_LSE_LIMIT:
                    try:
                        layer_name = getattr(self, "layer_name", "")
                        attn_f = attn_out.detach().float()
                        msg = (
                            f"DCP_LSE_DEBUG[{debug_count}] layer={layer_name} "
                            f"out_shape={tuple(attn_out.shape)} "
                            f"out_min={float(attn_f.amin().item()):.6g} "
                            f"out_max={float(attn_f.amax().item()):.6g} "
                            f"out_mean={float(attn_f.mean().item()):.6g}"
                        )
                        if lse is not None:
                            lse_f = lse.detach().float()
                            msg += (
                                f" lse_shape={tuple(lse.shape)} "
                                f"lse_min={float(lse_f.amin().item()):.6g} "
                                f"lse_max={float(lse_f.amax().item()):.6g} "
                                f"lse_mean={float(lse_f.mean().item()):.6g} "
                                f"lse_head={lse_f.flatten()[:8].cpu().tolist()}"
                            )
                        else:
                            msg += " lse=None"
                        logger.warning(msg)
                    except Exception:
                        logger.exception("DCP_LSE_DEBUG failed")
                    setattr(self, "_debug_dcp_lse_count", debug_count + 1)

            # correct dcp attn_out with lse.
            if self.impl.dcp_world_size > 1:
                if self.dcp_flashinfer_a2a:
                    attn_out = dcp_flashinfer_a2a_lse_reduce(
                        attn_out,
                        lse,
                        get_dcp_group(),
                ...
                elif os.getenv("VLLM_DCP_USE_LSE_ALL_REDUCE", "0") == "1":
                    ...
                    cp_num_heads = attn_out.shape[1] // cp_group.world_size
                ...
                if _DEBUG_DCP_GRAPH_STATS:
                    _record_debug_stats(self._debug_dcp_graph_stats, 12, attn_out)

            # v_up projection
            self._v_up_proj(attn_out, out=mqa_output_slice)
            if _DEBUG_DCP_GRAPH_STATS and self.impl.dcp_world_size > 1:
                _record_debug_stats(
                    self._debug_dcp_graph_stats,
                    16,
                    mqa_output_slice,
                )
...
class MLACommonMetadataBuilder(AttentionMetadataBuilder[M]):
    ...
    def __init__(
        ...
        # Skip DCP partitioning for spec-decode draft (eagle3) attention layers.
        # Draft attention is a tiny single-layer forward, but with DCP > 1 it
        # still pays the full all-gather/reduce-scatter overhead per step. On
        # 8x RTX PRO 6000 Blackwell + Kimi-K2.6 + kimi-k2.5-eagle3-mla we
        # measured this contributing ~6 ms per draft forward at DCP=8 — about
        # a quarter of the DCP+MTP performance gap. The deeper structural bug
        # (eagle3 spec-loop CPU sync per draft step, ~10 ms each) is out of
        # scope here; see /root/handoff_work/remote_dcp_mtp_brokenness.md §10.
        # Detection heuristic: if every layer in this builder is at an index
        # at or above the main model's num_hidden_layers, it's a draft-only
        # builder and DCP partitioning of its tiny KV is pure overhead.
        skip_draft_dcp = os.getenv("VLLM_MLA_DCP_SKIP_DRAFT_ATTN", "0") != "0"
        if skip_draft_dcp and self.dcp_world_size > 1 and layer_names:
            main_num_layers = getattr(
                vllm_config.model_config.hf_config, "num_hidden_layers", None
            )
            if main_num_layers is not None and main_num_layers > 0:
                ...
                idxs = [_layer_idx(n) for n in layer_names]
                if idxs and all(i >= main_num_layers for i in idxs):
                    logger.info(
                        "MLACommonMetadataBuilder: all %d layer(s) "
                        "(indices >= main num_hidden_layers=%d) appear to be "
                        "draft (eagle3) layers; overriding dcp_world_size "
                        "from %d to 1 to skip DCP all-gather/reduce-scatter "
                        "on the tiny draft forward.",
                        len(layer_names),
                        main_num_layers,
                        self.dcp_world_size,
                    )
...
class MLACommonImpl(MLAAttentionImpl[M], Generic[M]):
    ...
    def _run_prefill_new_tokens_trtllm_ragged(
        ...
        ret = trtllm_ragged_attention_deepseek(
            ...
            enable_pdl=_is_arch_support_pdl(),
            is_causal=True,
            return_lse=return_softmax_lse,
            out=out,
        )
