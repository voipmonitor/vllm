...
# Sparse MLA backends where overlapping the outer MLA/indexer GEMMs is useful.
# Other backends stay conservative unless explicitly enabled.
_SPARSE_MLA_BACKENDS_AUTO_FANOUT = ("FLASHMLA_SPARSE", "B12X_MLA_SPARSE")


def _aux_stream_list() -> list[torch.cuda.Stream]:
    stream = aux_stream()
    ...
    def __init__(
        ...
        if self._mla_preattn_fanout or self._mla_dense_midattn_fanout:
            if aux_stream() is None:
                self._mla_preattn_fanout = False
                self._mla_midattn_fanout = False
                self._mla_dense_midattn_fanout = False
    ...
    def forward(
        ...
        if self.q_lora_rank is not None:
            ...
            if self._mla_preattn_fanout:
                ...
                assert self._fanout_start_event is not None
                assert self._fanout_done_event is not None
            ...
            if self._mla_midattn_fanout:
                indexer = self.indexer
                assert indexer is not None
                assert self._fanout2_start_event is not None
                assert self._fanout2_done_event is not None
            ...
            elif self._mla_dense_midattn_fanout:
                assert self._fanout_dense_start_event is not None
                assert self._fanout_dense_done_event is not None
